...
% ----------------------------------------------------------------------- %
function line = plot_areaerrorbar(data, options)
    ...
    if (~isfield(options, 'linespec')), options.linespec = '-'; end 
    ...
    hold on;
    line = plot(options.x_axis, data_mean, 'color', options.color_line, ...
        'LineWidth', options.line_width,'LineStyle', options.linespec);
    %hold off;
